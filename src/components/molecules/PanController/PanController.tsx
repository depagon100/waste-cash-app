...
export class PanController extends React.Component<any, any> {
  _responder: any = null;
  _listener: any = null;
  _direction: any = null;
  deceleration;
    ...
    this._responder = PanResponder.create({
      ...
      onPanResponderRelease: (_, { vx, vy, dx, dy }) => {
        ...
        if (this.props.onRelease) {
          cancel = this.props.onRelease({ dx, dy, vx, vy }) === false;
        }

        if (!cancel && horizontal && (!lockDirection || dir === 'x')) {
          let [xMin, xMax] = xBounds;
          if (this.props.onReleaseX) {
            cancel = this.props.onReleaseX({ dx, dy, vx, vy }) === false;
          }
          !cancel &&
        ...
        if (!cancel && vertical && (!lockDirection || dir === 'y')) {
          ...
          if (this.props.onReleaseY) {
            cancel = this.props.onReleaseY({ dx, dy, vx, vy }) === false;
          }
          !cancel &&
            this.handleResponderRelease(
              panY,
